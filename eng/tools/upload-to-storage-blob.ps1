...
function Process-DocJS
{
    ...
    foreach ($Item in $PublishedDocs) {
        ...
        if (($PkgFullName | Measure-Object).count -eq 1) 
        {
            ...
            Upload-Blobs -DocDir "$($PipelineWorkspace)/documentation/$($Item.BaseName)/$($Item.BaseName)/$($DocVersion)" -PkgName $PkgName -DocVersion $DocVersion -SAS "?sv=2018-03-28&ss=bfqt&srt=sco&sp=rlacup&st=2019-10-10T21%3A23%3A56Z&se=2019-10-11T21%3A23%3A56Z&sig=BvRBlHuJoqHRPMciZNsCoSliPfJotGMywnnYBjmjwhU%3D" -Language "javascript"
        }
        else
        {
