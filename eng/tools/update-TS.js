...
const commitChanges = async (rushPackages, package) => {
  ...
  try {
    ...
    await writeFile(
      ...
      JSON.stringify(rushPackages[package].json, null, 2)
    );
    console.log(
      `File ${rushPackages[package].src} version updated to ${rushPackages[package].json.version}`
    );
  } catch (e) {
    console.error(e);
  }
...
const updateTSVersion = (rushPackages, package, buildId) => {
  dependencySection = rushPackages[package].json.devDependencies;
  dependencySection["typescript"] = "3.7.0-beta";
};

const main = async () => {
  const buildId = parseArg();
  const rushPackages = await getRushPackages(
  ...
  for (const package of Object.keys(rushPackages)) {
    updateTSVersion(rushPackages, package, buildId);
